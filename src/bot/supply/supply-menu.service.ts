...
Каждый час бот забирает с Додо ИС складские остатки. Далее он сверяет какой расход будет в текущий день (учитываются как праздничные дни, так и будни). Если сырья осталось менее чем минимальная норма это считается проблемой и формируется соответствующее сообщение. 

Например, в пиццерии средний расход сыра моцарелла в будни 50 килограмм. 
Минимальная норма 30 килограмм.
Если в 16:30 вечера сыра моцареллы останется меньше 30 килограмм, то будет создано соответствующее сообщение о проблеме.
...
В настройках бота возможно настроить время уведомлений, в какие пиццерии рассылать уведомления, какой минимальный остаток и прочее. Все настройки делаются на сайте dodo-sky.ru по  учетной записью с правами менеджера офиса.

<b>Вопрос 3.</b> Что за время указано вначале сообщения?
  Данные по остатку собираются непосредственно с Dodo IS. При этом Dodo IS выдает данные один раз в час, и соответственно в сообщение указывается на какое время актуален остаток по сырью. При этом время сообщения и время остатка может различаться.
