...
//   private readonly logger = new Logger(MySalaryService.name);
...
//   async showSalary(ctx: Context): Promise<void> {
//     try {
...
//       const premiumPayments = await this.getPremiumPayments(userId);
...
//   private async getPremiumPayments(idMax: number): Promise<PremiumPaymentResponse[]> {
//     try {
...
//           datePremium: { gte: tenDaysAgo },
//         },
//         orderBy: { datePremium: 'desc' },
//       });
...
//       // Затем группируем по namePremium внутри каждой даты и суммируем суммы
...
//             sourceId: '', // не нужен для сгруппированных данных
//             sourceType: '', // не нужен для сгруппированных данных
//             amount: totalAmount.toFixed(2),
//             namePremium: type,
//             datePremium: payments[0].datePremium, // берём первую дату из группы
//             formattedDate: date,
//           });
//         });
//       });
...
//     } catch (error) {
//       this.logger.error('Ошибка при получении доплат курьера:', error);
//       throw error;
//     }
//   }
...
//     message += `Дата начисления: ${payment.datePremium.toLocaleString('ru-RU', {
//       day: '2-digit',
//       month: '2-digit',
//       year: 'numeric',
//     })}\n`;
